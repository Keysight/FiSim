...
and MemReadHook = delegate of Unicorn * UInt64 * UInt32 * Object -> unit
and MemWriteHook = delegate of Unicorn * UInt64 * UInt32 * UInt64 * Object -> unit
and EventMemHook = delegate of Unicorn * Int32 * UInt64 * UInt32 * UInt64 * Object -> Boolean
...
and Unicorn(arch: Int32, mode: Int32, binding: IBinding) = 
    ...
    let _internalHookList = new List<Object>()
    ...
    member this.MemMap(address: UInt64, size: UInt64, perm: Int32) =
        let size = new UIntPtr(uint64 size)
        match binding.MemMap(_eng.[0], uint64 address, size, uint32 perm) |> this.CheckResult with 
        | Some e -> raise e | None -> ()

    member this.MemMapPtr(address: UInt64, size: UInt64, perm: Int32, ptr: IntPtr) =
        let size = new UIntPtr(uint64 size)
    ...
    member this.MemUnmap(address: UInt64, size: Int64) =
        let size = new UIntPtr(uint64 size)
        match binding.MemUnmap(_eng.[0], uint64 address, size) |> this.CheckResult with 
        | Some e -> raise e | None -> ()   

    member this.MemProtect(address: UInt64, size: Int64, ?perm: Int32) =
        let size = new UIntPtr(uint64 size)
    ...
    member this.RegWrite(regId: Int32, value: UInt64) =
        this.RegWrite(regId, uint64ToBytes value)
    ...
    member this.RegRead(regId: Int32) =
        ...
        bytesToUInt64 buffer

    member this.EmuStart(beginAddr: UInt64, untilAddr: UInt64, timeout: UInt64, count: UInt64) =
    ...
    member this.AddCodeHook(callback: CodeHook, userData: Object, beginAddr: UInt64, endAddr: UInt64) =
        let trampoline(u: IntPtr) (addr: UInt64) (size: UInt32) (user: IntPtr) =
            callback.Invoke(this, addr, size, userData)
            
        let internalHook = new CodeHookInternal(trampoline)
        let funcPointer = Marshal.GetFunctionPointerForDelegate(internalHook)        
        let hh = new UIntPtr(allocate(IntPtr.Size))
        
        _internalHookList.Add(internalHook)
    ...
    member this.AddCodeHook(callback: CodeHook, beginAddr: UInt64, endAddr: UInt64) =
    ...
    member this.AddBlockHook(callback: BlockHook, userData: Object, beginAddr: Int64, endAddr: Int64) =   
        ...
        if _blockHooks |> Seq.isEmpty then
            let internalHook = new BlockHookInternal(trampoline)
            let funcPointer = Marshal.GetFunctionPointerForDelegate(internalHook)     
            let hh = new UIntPtr(allocate(IntPtr.Size))
    ...
    member this.AddInterruptHook(callback: InterruptHook, userData: Object) =   
        ...
        if _interruptHooks |> Seq.isEmpty then
            let internalHook = new InterruptHookInternal(trampoline)
            let funcPointer = Marshal.GetFunctionPointerForDelegate(internalHook)
            let hh = new UIntPtr(allocate(IntPtr.Size))
    ...
    member this.AddMemReadHook(callback: MemReadHook, userData: Object, beginAddr: UInt64, endAddr: UInt64) =
        let trampoline(u: IntPtr) (eventType: Int32) (addr: UInt64) (size: UInt32) (user: IntPtr) =
            callback.Invoke(this, addr, size, userData)
               
        let internalHook = new MemReadHookInternal(trampoline)
        let funcPointer = Marshal.GetFunctionPointerForDelegate(internalHook)        
        let hh = new UIntPtr(allocate(IntPtr.Size))
    ...
    member this.AddMemWriteHook(callback: MemWriteHook, userData: Object, beginAddr: UInt64, endAddr: UInt64) =   
        let trampoline(u: IntPtr) (eventType: Int32) (addr: UInt64) (size: UInt32) (value: UInt64) (user: IntPtr) =
            callback.Invoke(this, addr, size, value, userData)
               
        let internalHook = new MemWriteHookInternal(trampoline)
        let funcPointer = Marshal.GetFunctionPointerForDelegate(internalHook)        
        let hh = new UIntPtr(allocate(IntPtr.Size))
    ...
    member this.AddEventMemHook(callback: EventMemHook, eventType: Int32, userData: Object) =
        ...
        let internalHook = new EventMemHookInternal(trampoline)
        let funcPointer = Marshal.GetFunctionPointerForDelegate(internalHook)        
        let hh = new UIntPtr(allocate(IntPtr.Size))
        
        _internalHookList.Add(internalHook)
        
        match binding.HookAddArg0Arg1(_eng.[0], hh, eventType, new UIntPtr(funcPointer.ToPointer()), IntPtr.Zero, 0UL, 0xFFFFFFFFFFFFFFFFUL) |> this.CheckResult with 
        | Some e -> raise e | None -> ()
...
//        |> Seq.iter(fun eventFlag -> _memEventHooks.[eventFlag].Add((callback, userData)))
    ...
    member this.AddInHook(callback: InHook, userData: Object) =
        ...
        if _inHooks |> Seq.isEmpty then
            let internalHook = new InHookInternal(trampoline)
            let funcPointer = Marshal.GetFunctionPointerForDelegate(internalHook)
            let hh = new UIntPtr(allocate(IntPtr.Size))
    ...
    member this.AddOutHook(callback: OutHook, userData: Object) =
        ...
        if _outHooks |> Seq.isEmpty then
            let internalHook = new OutHookInternal(trampoline)
            let funcPointer = Marshal.GetFunctionPointerForDelegate(internalHook)
            let hh = new UIntPtr(allocate(IntPtr.Size))
    ...
    member this.AddSyscallHook(callback: SyscallHook, userData: Object) =
        ...
        if _syscallHooks |> Seq.isEmpty then
            let internalHook = new SyscallHookInternal(trampoline)
            let funcPointer = Marshal.GetFunctionPointerForDelegate(internalHook)
            let hh = new UIntPtr(allocate(IntPtr.Size))
